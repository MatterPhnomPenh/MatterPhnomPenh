...
export default function MatterFest() {
  ...
  const festVariants = {
    ...
    visible: { opacity: 0.5, y: 0, transition: { duration: 0.8, ease: "easeOut" } },
  };

  return (
    <section id="fest" className="relative flex flex-col items-center justify-center w-full min-h-screen bg-[#2330ff] text-white overflow-hidden px-4 md:px-0">
      <div className="flex flex-col md:flex-row items-center justify-center w-full max-w-6xl text-center md:text-left">
        {/* Text Section */}
        <motion.div
          initial="hidden"
          animate="visible"
          variants={textVariants}
          className="w-full md:w-1/2 md:px-12 z-10"
        >
          <h1 className={`${isMobile ? "text-3xl" : "text-5xl"} font-extrabold mb-6`}>
        ...
        {/* Album Stack */}
        <div
          className={`relative ${isMobile ? "w-[250px]" : "w-[300px] md:w-[400px]"} aspect-square mt-8 md:mt-0 z-10`}
        >
          {/* Back Image - rotated */}
          <motion.div
            initial={{ rotate: -8, x: isMobile ? 0 : -20, y: isMobile ? 0 : 20 }}
            animate={{ rotate: -8, x: isMobile ? 0 : -20, y: isMobile ? 0 : 20 }}
          ...
          >
            <div className="relative w-full h-full overflow-hidden rounded-lg shadow-2xl">
              <Image src={albums[currentImageIndex].image} alt="" fill className="object-cover" />
            </div>
          </motion.div>
        </div>
      </div>
      ...
      <motion.h1
        ...
        className={`absolute ${isMobile ? "top-8 left-1/2 -translate-x-1/2" : "top-4 right-4 md:right-50"} ${isMobile ? "text-[80px]" : "text-[160px] md:text-[220px]"} font-bold text-white leading-none select-none z-0`}
      >
        Fest
